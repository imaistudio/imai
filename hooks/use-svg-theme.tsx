...
export function useSVGTheme() {
  ...
  useEffect(() => {
    ...
    const updateSVGColors = () => {
      ...
      const productPlaceholderImages = document.querySelectorAll('img[src*="/inputs/placeholders/"][src$=".svg"]');
        ...
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              const images = element.querySelectorAll?.('img[src*="/inputs/placeholders/"][src$=".svg"]');
