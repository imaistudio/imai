...
const TOOL_PARAMETERS: Record<string, Record<string, string>> = {
  ...
  "videosound": {
    "prompt": "Generate realistic ambient and foreground sounds that match the visual content, timing, environment, and actions in the video. Ensure the audio reflects the correct atmosphere, object interactions, materials, spatial depth, and motion. Maintain temporal alignment and avoid adding unrelated sounds.",
    "original_sound_switch": "false",
  },
};

export async function handleToolcall(formData: FormData): Promise<NextResponse> {
  console.log("🛠️ TOOLCALL ROUTER: Processing direct tool request");
  
  try {
    const toolcall = formData.get("toolcall") as string;
    const userid = formData.get("userid") as string;
    const productPath = formData.get("product") as string || formData.get("product_image_url") as string;
    ...
    if (expectsJson) {
      ...
      if (toolcall === "clarityupscaler") {
        requestBody = JSON.stringify({
          imageUrl: jsonData.image_url || jsonData.product_image_url || jsonData.product,
