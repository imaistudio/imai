...
// Simple in-memory cache for enhanced prompts (production would use Redis)
const enhancementCache = new Map<string, {
  enhanced_prompt: string;
  enhancement_type: string;
  tokens_used: number;
  timestamp: number;
...
// Cache cleanup interval (remove entries older than 1 hour)
setInterval(() => {
...
/**
 * Enhances a user prompt using Claude 3.5 Sonnet for optimal speed and quality
 */
async function enhancePromptWithClaude(
  ...
  try {
    // Calculate target word count (4x the input)
    ...
    const response = await anthropic.messages.create({
      model: "claude-3-5-sonnet-20241022", // Faster than Claude 4 Sonnet
      max_tokens: 512, // Reduced from 1024 for faster response
      temperature: 0.7, // Optimized for speed and creativity
      system: systemPrompt,
      messages: [
        {
          role: "user",
...
/**
 * Returns optimized system prompt based on enhancement type (concise for speed)
 */
...
/**
 * Optimized enhancement type detection (faster keyword matching)
 */
function detectEnhancementType(
  ...
  // Quick image-based detection first
  if (hasProductImage) return 'product';
  if (hasDesignImage) return 'design';

  const lowerPrompt = prompt.toLowerCase();
  
  // Optimized keyword detection with early returns
  // Technical keywords (highest priority for technical content)
  if (/\b(blueprint|technical|engineering|architectural|cad|schematic|diagram)\b/.test(lowerPrompt)) {
    return 'technical';
  }

  // Product keywords
  if (/\b(product|commercial|e-commerce|catalog|marketing|brand|retail)\b/.test(lowerPrompt)) {
    return 'product';
  }

  // Artistic keywords
  if (/\b(artistic|creative|abstract|painting|art|gallery|mood|dramatic|painterly)\b/.test(lowerPrompt)) {
    return 'artistic';
  }

  // Design keywords (default fallback)
  return 'design';
}

/**
 * Validates the enhancement type parameter
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    // Parse FormData from intent route
    const formData = await request.formData();
    
    // 1) Extract and validate userid (required parameter from intent route)
    ...
    if (originalPrompt.length > 2000) {
      return NextResponse.json(
        { status: 'error', error: 'Prompt too long. Maximum 2000 characters allowed.' },
        { status: 400 }
      );
    }

    // 3) Extract optional image context (for better enhancement type detection)
    const hasProductImage = (formData.get('has_product_image') as string) === 'true';
    const hasDesignImage = (formData.get('has_design_image') as string) === 'true';
    const hasColorImage = (formData.get('has_color_image') as string) === 'true';

    // 4) Automatically detect enhancement type (or use provided one as override)
    ...
    console.log('🤖 Auto-detected enhancement type:', enhancementType, 'for prompt:', originalPrompt.substring(0, 50) + '...');

    // 5) Calculate word counts for targeted enhancement
    const inputWordCount = originalPrompt.trim().split(/\s+/).length;
    const targetWordCount = inputWordCount * 4;
    console.log(`📊 Word count: ${inputWordCount} → ${targetWordCount} (4x)`);

    // 6) Check cache first for faster response
    ...
    // 7) Enhance the prompt using Claude 3.5 Sonnet
    const enhancementResult = await enhancePromptWithClaude(originalPrompt, enhancementType);
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    // This GET endpoint provides information about the prompt enhancer API
    const apiInfo = {
      status: 'active',
      description: 'Prompt Enhancer API using Claude 3.5 Sonnet with automatic enhancement type detection',
      ...
      supported_enhancement_types: [
        {
          ...
          keywords: 'design, modern, contemporary, sleek, elegant, sophisticated'
        },
        {
          type: 'product',
          description: 'Commercial product photography enhancement',
          keywords: 'product, commercial, e-commerce, catalog, marketing, brand'
        },
        {
          type: 'artistic',
          description: 'Artistic and creative expression enhancement',
          keywords: 'artistic, creative, abstract, expressive, painterly, stylized'
        },
        {
          type: 'technical',
          description: 'Technical and architectural visualization enhancement',
          keywords: 'blueprint, technical, engineering, architectural, CAD, schematic'
        },
        {
          type: 'general',
      ...
      usage: {
        ...
        optional_fields: ['enhancement_type', 'has_product_image', 'has_design_image', 'has_color_image'],
      ...
      example_response: {
        ...
        enhanced_prompt: 'A sleek, contemporary ceramic coffee mug with clean geometric lines...',
