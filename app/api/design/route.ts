...
console.log("🔥 Firebase disabled in design route - using intentroute for file handling");
...
async function uploadBufferToFirebase(
	...
	try {
		if (!firebaseInitialized) {
			throw new Error("Firebase is not initialized - cannot upload to Firebase Storage");
		}
...
async function resizeImageIfNeeded(imageUrl: string): Promise<string> {
	try {
		...
		try {
			...
			console.warn('Sharp not available or failed, using original image:', sharpError);
			return imageUrl;
		}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
	try {
		...
		const presetProductType = formData.get("preset_product_type") as string | null;
		const presetDesignStyle = formData.get("preset_design_style") as string | null;
		const presetColorPalette = formData.get("preset_color_palette") as string | null;
		...
		const hasProduct = !!productImage || !!productImageUrl || !!presetProductType;
