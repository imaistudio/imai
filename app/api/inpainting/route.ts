 ...
 */
async function processMask(maskDataUrl: string, targetWidth?: number, targetHeight?: number): Promise<Buffer> {
  ...
  if (targetWidth && targetHeight) {
    ...
    console.log(`🎭 Resized mask to ${targetWidth}x${targetHeight} (aspect ratio preserved)`);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const options: InpaintingOptions = {
      size: (formData.get("size") as "256x256" | "512x512" | "1024x1024") || "1024x1024",
    ...
    const requestedSize = formData.get("size") as "256x256" | "512x512" | "1024x1024";
    if (requestedSize) {
      targetSize = requestedSize;
    }
    
    console.log(`🎯 Original size: ${width}x${height}, aspect ratio: ${aspectRatio.toFixed(2)}, using: ${targetSize}`);
    ...
    console.log(`🖼️ Resized image to ${targetDimension}x${targetDimension} (aspect ratio preserved)`);

    // Process the mask - ensure it matches the target size
    console.log("🎭 Processing mask...");
    const maskBuffer = await processMask(maskDataUrl, targetDimension, targetDimension);
    
    // Debug: Check mask validity
    const maskStats = await sharp(maskBuffer).stats();
    const hasWhitePixels = maskStats.channels.some(channel => channel.max > 200);
    console.log(`🔍 Mask validation: ${hasWhitePixels ? 'Valid (has white pixels)' : 'INVALID (no white pixels found)'}`);
    ...
    const response = await client.images.edit({
      ...
      image: await toFile(resizedImageBuffer, "image.png", { type: "image/png" }),
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json({
    ...
    endpoints: {
      POST: {
        description: "Edit an image using a mask to guide the inpainting process",
        parameters: {
          userid: "string (required) - User ID",
          image_url: "string (required) - URL of the image to edit",
          mask: "string (required) - Base64 data URL of the mask image",
          prompt: "string (required) - Description of what to paint in the masked area",
          size: "string (optional) - Output size (default: 1024x1024)",
          quality: "string (optional) - Output quality: standard/hd (default: standard)",
