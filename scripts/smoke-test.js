...
async function executeTestCase(testCase, concurrent = false) {
  ...
  try {
    ...
    if (success) {
      ...
      console.log(
        `   Status: ${response.status}, Service: ${testCase.service}`,
      );
      if (result.status) console.log(`   Result: ${result.status}`);
    } else {
      console.log(`❌ ${testId} - FAILED (${responseTime}ms)`);
      console.log(
        `   Status: ${response.status}, Error: ${result.error || result.message || "Unknown error"}`,
      );
      if (result.details)
...
async function testQueueFunctionality() {
  ...
  for (
    let batch = 0;
    batch < Math.ceil(queueTests.length / QUEUE_TEST_CONFIG.batchSize);
    batch++
  ) {
    const batchStart = batch * QUEUE_TEST_CONFIG.batchSize;
    const batchTests = queueTests.slice(
      batchStart,
      batchStart + QUEUE_TEST_CONFIG.batchSize,
    );

    console.log(
      `📦 Batch ${batch + 1}: Testing ${batchTests.length} concurrent requests...`,
    );
    ...
    for (
      let i = 0;
      i < QUEUE_TEST_CONFIG.concurrentRequests && i < batchTests.length;
      i++
    ) {
    ...
    if (
      batch <
      Math.ceil(queueTests.length / QUEUE_TEST_CONFIG.batchSize) - 1
    ) {
      console.log(
        `⏱️  Waiting ${QUEUE_TEST_CONFIG.delayBetweenBatches}ms before next batch...`,
      );
      await new Promise((resolve) =>
        setTimeout(resolve, QUEUE_TEST_CONFIG.delayBetweenBatches),
      );
    }
  }
...
function displayResults(sequentialResults, queueResults = []) {
  ...
  if (sequentialPassed < sequentialTotal) {
    ...
    sequentialResults
      .filter((r) => !r.success)
      .forEach((r) =>
        console.log(`   - ${r.testCase.name}: ${r.error || "API error"}`),
      );
  }
  ...
  Object.entries(serviceStats).forEach(([service, stats]) => {
    ...
    console.log(
      `   ${service.toUpperCase()}: ${stats.passed}/${stats.total} (${percentage}%)`,
    );
  });

  // Overall results
  const totalPassed =
    sequentialPassed + queueResults.filter((r) => r.success).length;
  const totalTests = sequentialTotal + queueResults.length;
  const overallPercentage = Math.round((totalPassed / totalTests) * 100);

  console.log("\n🎯 Overall Results:");
  console.log(
    `✅ ${totalPassed}/${totalTests} tests passed (${overallPercentage}%)`,
  );

  if (overallPercentage >= 80) {
    console.log("\n🎉 Smoke Test PASSED! APIs are functioning well.");
    console.log(
      "🚀 System ready for load testing with 25-100 concurrent users.",
    );
    ...
    console.log(
      "🔧 Consider fixing failed endpoints before full load testing.",
    );
  } else {
    console.log("\n❌ Smoke Test FAILED. Major issues detected.");
    console.log(
      "🛠️  Fix critical endpoints before proceeding with load testing.",
    );
  }
  ...
  if (queueResults.length === 0) {
    console.log(
      "   • Run with queue testing: node scripts/smoke-test.js --queue",
    );
  }
  console.log("   • Monitor rate limiting behavior during high load");
  console.log(
    "   • Test with different image/video URLs for comprehensive coverage",
  );
  console.log(
    "   • Consider implementing health check endpoints for faster monitoring",
  );
}
...
if (require.main === module) {
  console.log("Usage:");
  console.log(
    "  node scripts/smoke-test.js           # Basic sequential tests",
  );
  console.log(
    "  node scripts/smoke-test.js --queue   # Include concurrent/queue testing",
  );
